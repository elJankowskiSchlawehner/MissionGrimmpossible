...
public class PlayfieldInitialiser : MonoBehaviour
{
    ...
    */
    ...
    {
        // Spielerfigur setzen
        Vector3 player_V = SPAWNER_V;
        player_V.x += (WidthPlayfield / 2) * TileWidth;
        player_V.y += (TilePrefab.transform.localScale.y / 2);
        player_V.z -= TileHeight;

        Vector3 playerSpawnV = new Vector3(player_V.x, player_V.y + PlayerSpawnHeight, player_V.z);
        GameObject player = Instantiate(Player, playerSpawnV, Quaternion.identity);

        // Spielfigur - Einstellungen vornehmen
        player.name = "Player";
        player_V.y += player.transform.localScale.y;
        player.GetComponent<Player>().ResetPoint_V = player_V;
    ...
    private void CreateAmbience(Vector3 position_V)
    {
        ...
        rearwall.GetComponent<Renderer>().material = WallTexture;
    ...
    public int GetWidthPlayfield()
    {
        return WidthPlayfield;
    }

    public int GetHeightPlayfield()
    {
        return HeightPlayfield;
    }
}